...
document.getElementById("formulario").addEventListener("submit", function (e) {
    ...
    if (
        ...
        const funcoes = ["DJ BT", "32⬆️⬇️", "67⬆️⬇️", "67-N⬆️⬇️", "51v", "27", "59", "81U", "81O", "46", "47", "25"];
        funcoes.forEach(funcao => {
            const circulo = document.createElement("div");
            circulo.className = "circulo";
            circulo.style.fontSize = "16px";
...
function reprovadorPesquisarPorTopico() {
  const topico = document.getElementById('reprovador-pesquisa-topico').value;
  const resultados = document.getElementById('reprovador-resultados');
  resultados.innerHTML = '';
  if (!topico) {
    reprovadorPesquisar(); // Se não selecionou tópico, faz pesquisa normal
    return;
  }
  supabaseClient
    .from('reprovacoes')
